            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div className="space-y-2">
                    {navItems.map(({ label, icon: Icon }) => (
                        <button
                            ...
                            className={`w-full text-left px-4 py-3 rounded-xl text-sm font-bold transition-all flex items-center gap-3 ${label === activeTab
                ...
                <div className="md:col-span-2 space-y-8 min-h-[500px]">
                        ...
                        <section className="space-y-6 animate-in slide-in-from-bottom-5 duration-500">
                            ...
                            <div className={cardCls}>
                                ...
                                <form onSubmit={handleUpdateProfile} className="space-y-6">
                                    ...
                                    {message && activeTab === 'Profile Information' && (
                    ...
                    {activeTab === 'Security & Privacy' && (
                        <section className="space-y-6 animate-in slide-in-from-bottom-5 duration-500">
                            <h3 className="text-sm font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Security & Privacy</h3>
                            <div className={cardCls}>
                                <div className="space-y-8">
                                    <div className="flex items-center gap-4 pb-6 border-b border-slate-100 dark:border-slate-800">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-500 dark:text-slate-400 font-medium italic">Update your account credentials.</p>
                                        </div>
                                    </div>
                                    ...
                                    <form onSubmit={handleUpdatePassword} className="space-y-6 max-w-md">
                                        <div className="space-y-1">
                                            ...
                                            <div className="relative">
                                                ...
                                                <input className="input-modern !pl-10" type="password" required value={currentPassword} onChange={e => setCurrentPassword(e.target.value)} />
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            ...
                                            <div className="relative">
                                                <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                                <input className="input-modern !pl-10" type="password" required value={newPassword} onChange={e => setNewPassword(e.target.value)} />
                                            </div>
                                        </div>
                                        <div className="space-y-1">
                                            ...
                                            <div className="relative">
                                                <Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                                <input className="input-modern !pl-10" type="password" required value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} />
                                            </div>
                                        </div>

                                        {message && activeTab === 'Security & Privacy' && (
                                            <div className={`flex items-center gap-3 p-4 rounded-xl text-sm font-bold ${message.type === 'success'
                                                ? 'bg-emerald-50 dark:bg-emerald-950/20 text-emerald-600 border border-emerald-100 dark:border-emerald-800'
                                                : 'bg-red-50 dark:bg-red-950/20 text-red-600 border border-red-100 dark:border-red-800'
                                                }`}>
                                        ...
                                        <div className="flex items-center justify-between opacity-50 cursor-not-allowed">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-slate-500 dark:text-slate-400 font-medium italic">Enabled for enhanced account security.</p>
                                                </div>
                                            </div>
                                            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Manage</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    {activeTab === 'Notifications' && (
                        <section className="space-y-6 animate-in slide-in-from-bottom-5 duration-500">
                            <h3 className="text-sm font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">Notifications</h3>
                            <div className={cardCls}>
                                <div className="space-y-8">
                                    <div className="flex flex-col items-center justify-center py-12 text-center space-y-4">
                                        <div className="p-4 bg-violet-600/10 rounded-full">
                                            <Bell className="w-8 h-8 text-violet-600 animate-bounce" />
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-lg font-bold text-slate-900 dark:text-white">Email Notifications</p>
                                            <p className="text-sm text-slate-500 dark:text-slate-400 max-w-xs">Get system updates, announcements, and activity reports delivered to your inbox.</p>
                                        </div>
                                        <button className="btn-primary !rounded-full px-8">Enable Notifications</button>
                                    </div>

                                    <div className="grid grid-cols-1 gap-4 pt-8 border-t border-slate-100 dark:border-slate-800">
                                        {[
                                            { id: 'system', label: 'System Updates', desc: 'Updates about the platform and new features.' },
                                            { id: 'course', label: 'Course Announcements', desc: 'Notifications from your enrolled subjects.' },
                                            ...
                                            <div key={i} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/50">
                                                ...
                                                <button
                                                    ...
                                                    className={`w-10 h-5 rounded-full relative transition-colors ${notifSettings[item.id as keyof typeof notifSettings] ? 'bg-violet-600' : 'bg-slate-200 dark:bg-slate-700'}`}
                                                >
                                                    <div className={`absolute top-1 w-3 h-3 bg-white rounded-full transition-all ${notifSettings[item.id as keyof typeof notifSettings] ? 'right-1' : 'left-1'}`}></div>
                                                </button>
                                            </div>
                                        ))}
                                    </div>
